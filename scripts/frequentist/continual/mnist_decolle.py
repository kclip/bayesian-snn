...
from optim.EWC import compute_fischer, update_previous_params_list
...
if __name__ == '__main__':
    ...
    results_path = make_experiment_dir(args.home + '/results',
                                       'mnist_freq_nepochs_%d_dataset_'
    ...
    for ite in range(args.num_ite):
        ...
        for i, digits in enumerate(tasks):
            ...
            tasks_seen += [digits]
            fim_list = compute_fischer(net, train_dl, loss_fn,
                                       optimizer, device, fim_list)
            previous_params = update_previous_params_list(net, previous_params)
